...
import {createDefaultLights, config} from '../../utils';
...
class MainScript extends Sein.LevelScriptActor<LevelState> {
  ...
  public onUpdate() {
    ...
    this.getGame().event.trigger('UpdateState', {hp: state.hp, killNum: this.getLevel<LevelState>().state.killNum});
    if (state.hp === 0) {
      this.getGame().pause();
    } else {
    }
