...
function convert(distRootDir) {
  ...
  let distPageAxml = `  <seinjs bindSeinCanvasCreated="onSeinCanvasCreated" />
    <sidebar bindSelect="onSelectDemo" current="{{current}}" />`;
  ...
  config.forEach(category => {
    ...
    if (only && only.indexOf('wx-mini-program') < 0) {
      return;
    }
    ...
    list.forEach(({id, name, pathFromHomepage}) => {
      ...
      distPageAxml += `\n  <${compName}
      id="${compName}"
      wx:if="{{current === '${compName}'}}"
      bindGameCreated="onGameCreated"
    />`
  ...
  fs.writeFileSync(path.resolve(distPagePath, 'index.wxml'), `<view>
  ${distPageAxml}
  </view>
  `);
...
const distRootDir = path.resolve(__dirname, '../../wx-mini-program');
convert(distRootDir);
